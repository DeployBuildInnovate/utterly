...
const TUTORS = [
  {
    ...
    firstName: 'Sarah',
    city: 'Leeds',
    bio: 'Warm, patient, and rigorous. I specialise in pronunciation and everyday conversation for adult learners.',
    ...
    teaches: ['A1', 'A2', 'B1', 'B2'],
    reviewList: [
      { name: 'Agnieszka W.', country: 'Poland', text: 'Sarah transformed my pronunciation in just six weeks. I can hear the difference myself now.', rating: 5, date: 'March 2026' },
  ...
  },
  {
    ...
    longBio: 'I spent fifteen years as a journalist and producer at the BBC before moving into teaching. That background shapes everything about how I teach. I am obsessed with clarity — saying exactly what you mean, in the right register, for the right audience. If you work in a professional environment and need to write better emails, present more confidently, or simply sound more authoritative in meetings, I am the tutor for you. I hold DELTA and CELTA and have been teaching professional British English for fourteen years.',
  ...
  },
  {
    ...
    longBio: 'I have been teaching English in Manchester for four years and I specialise in learners who are just starting out or who have tried before and lost confidence. My sessions are gentle, encouraging, and always well prepared. I never make learners feel embarrassed about mistakes — mistakes are how we learn. I hold a TEFL qualification and am currently working toward my CELTA.',
    certifications: ['TEFL'],
    experience: 4,
    rating: 4.7,
    reviews: 41,
  ...
  },
  {
    ...
    longBio: 'I teach from Edinburgh and I specialise in spoken fluency — getting learners to think and speak in British English naturally and quickly. My sessions are fast-paced and interactive. We use real scenarios, role plays, and a lot of conversation. I love British idiom and informal language and I think it is one of the most enjoyable parts of learning the language.',
    certifications: ['CELTA'],
    experience: 6,
    rating: 4.8,
    reviews: 72,
  ...
  },
  {
    ...
    longBio: 'I teach from Birmingham and I host the 5pm Teatime Club every evening. I specialise in nervous learners and group teaching — creating an environment where people feel comfortable enough to actually speak. My sessions are relaxed, warm, and always encouraging. I hold TEFL and CELTA qualifications and have been teaching for five years.',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '3rem', padding: '2rem 0 4rem', alignItems: 'start' }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1.5rem', marginBottom: '1.5rem' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: '14px', color: '#666' }}>{tutor.lessons.toLocaleString()} lessons</span>
          ...
          <div style={{ marginBottom: '2rem' }}>
            ...
            <div style={{ width: '100%', aspectRatio: '16/9', background: '#f5f5f0', border: '1px solid #e5e5e5', borderRadius: '12px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
              ...
              <p style={{ fontSize: '14px', color: '#999', margin: 0 }}>{tutor.firstName}&apos;s intro video</p>
            </div>
          </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                {tutor.specialisms.map((s: string) => (
                  <span key={s} style={{ fontSize: '13px', background: '#f5f5f0', border: '1px solid #e5e5e5', padding: '5px 12px', borderRadius: '20px', color: '#555' }}>{s}</span>
                ))}
              </div>
            </div>
            <div>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                {tutor.teaches.map((l: string) => (
                  <span key={l} style={{ fontSize: '13px', background: '#E1F5EE', color: '#0F6E56', padding: '5px 12px', borderRadius: '20px', fontWeight: '500' }}>{l}</span>
                ))}
              </div>
            </div>
          </div>
        ...
        <div style={{ position: 'sticky', top: '2rem' }}>
          <div style={{ border: '1px solid #e5e5e5', borderRadius: '16px', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '1.5rem', borderBottom: '1px solid #e5e5e5' }}>
              <h3 style={{ fontSize: '14px', fontWeight: '600', color: '#1a1a1a', marginBottom: '1rem', fontFamily: 'sans-serif' }}>Choose a day</h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px', marginBottom: '1rem' }}>
                {DAYS.map((day, i) => (
                  <button key={day} style={{ padding: '6px 4px', border: '1px solid #e5e5e5', borderRadius: '8px', fontSize: '11px', fontWeight: '500', cursor: 'pointer', background: i === 1 ? '#0F6E56' : 'white', color: i === 1 ? 'white' : '#444', fontFamily: 'sans-serif' }}>
                    {day}
                  </button>
                ))}
              </div>
              <h3 style={{ fontSize: '14px', fontWeight: '600', color: '#1a1a1a', marginBottom: '0.75rem', fontFamily: 'sans-serif' }}>Available times</h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '6px' }}>
                {TIMES.slice(0, 6).map((time, i) => (
                  <button key={time} style={{ padding: '8px', border: '1px solid #e5e5e5', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', background: i === 0 ? '#E1F5EE' : 'white', color: i === 0 ? '#0F6E56' : '#444', fontFamily: 'sans-serif', fontWeight: i === 0 ? '600' : '400' }}>
                    {time}
                  </button>
                ))}
              </div>
