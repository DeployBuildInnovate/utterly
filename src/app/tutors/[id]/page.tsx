...
export default function TutorsPage() {
  ...
  return (
    <main style={{ fontFamily: 'Georgia, serif', maxWidth: '1100px', margin: '0 auto', padding: '0 2rem' }}>
      ...
      {/* Nav — changes based on auth state */}
      {user ? (
        // Logged in nav
        <>
          <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '56px', borderBottom: '1px solid #e5e5e5' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
              ...
              <Link href="/tutors" style={{ textDecoration: 'none', fontSize: '14px', color: '#0F6E56', fontWeight: '600' }}>
                Find a tutor
              </Link>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <Link href="/help" style={{ textDecoration: 'none', padding: '8px', fontSize: '18px', color: '#444' }}>❓</Link>
              <Link href="/dashboard/learner/saved" style={{ textDecoration: 'none', padding: '8px', fontSize: '18px', color: '#444' }}>🤍</Link>
              <Link href="/club" style={{ textDecoration: 'none', padding: '8px', fontSize: '18px', color: '#7C3AED' }}>🍵</Link>
              <div style={{ position: 'relative' }}>
                ...
                {dropdown && (
                  <div style={{ position: 'absolute', top: '40px', right: 0, background: 'white', border: '1px solid #e5e5e5', borderRadius: '12px', padding: '8px', minWidth: '200px', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', zIndex: 200 }}>
                    <div style={{ padding: '8px 12px 12px', borderBottom: '1px solid #f5f5f0', marginBottom: '4px' }}>
                      ...
                      <p style={{ fontSize: '12px', color: '#999', margin: '2px 0 0', fontFamily: 'sans-serif' }}>{user.email}</p>
                    </div>
                    {[
                      { label: 'My dashboard', href: '/dashboard/learner' },
                      { label: 'My lessons', href: '/dashboard/learner' },
                      { label: 'Saved tutors', href: '/dashboard/learner/saved' },
                      { label: 'Settings', href: '/dashboard/learner' },
                    ].map(item => (
                      <Link key={item.label} href={item.href}
                        style={{ display: 'block', padding: '9px 12px', fontSize: '14px', color: '#444', textDecoration: 'none', borderRadius: '8px' }}
                    ...
                    <div style={{ borderTop: '1px solid #f5f5f0', marginTop: '4px', paddingTop: '4px' }}>
                      <button onClick={handleSignOut}
                        style={{ display: 'block', width: '100%', textAlign: 'left', padding: '9px 12px', fontSize: '14px', color: '#DC2626', background: 'none', border: 'none', cursor: 'pointer', borderRadius: '8px', fontFamily: 'sans-serif' }}>
                        Log out
                      </button>
                    </div>
                  </div>
          ...
          {/* Secondary nav for logged in */}
          <div style={{ display: 'flex', borderBottom: '1px solid #e5e5e5', marginBottom: '0' }}>
            {[
              { label: 'Home', href: '/dashboard/learner' },
              { label: 'Messages', href: '/dashboard/learner' },
              { label: 'My lessons', href: '/dashboard/learner' },
              { label: 'Settings', href: '/dashboard/learner' },
            ].map(item => (
              <Link key={item.label} href={item.href}
                style={{ textDecoration: 'none', padding: '0 1rem', height: '48px', display: 'flex', alignItems: 'center', fontSize: '14px', color: '#666', borderBottom: '2px solid transparent' }}>
                {item.label}
              </Link>
            ))}
          </div>
      ...
      <section style={{ padding: '2rem 0 1.5rem' }}>
        {user ? (
          <h1 style={{ fontSize: '28px', fontWeight: '600', letterSpacing: '-1px', marginBottom: '0.5rem', color: '#1a1a1a' }}>
            {TUTORS.length} British English tutors to help you succeed
          </h1>
        ) : (
          <>
            ...
            <h1 style={{ fontSize: '36px', fontWeight: '600', letterSpacing: '-1.5px', marginBottom: '0.75rem', color: '#1a1a1a' }}>
              Find your tutor
            </h1>
            <p style={{ fontSize: '15px', color: '#666', lineHeight: '1.6', maxWidth: '520px', margin: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexShrink: 0 }}>
                    <button
                      ...
                      style={{ background: 'none', border: '1px solid #e5e5e5', borderRadius: '8px', padding: '8px 12px', fontSize: '16px', cursor: 'pointer', lineHeight: 1 }}
                    >
                      {saved.includes(tutor.id) ? '❤️' : '🤍'}
                    </button>
